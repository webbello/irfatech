 ...
 */
import { localizedPath, defaultLocale, getLocales, getEntryLocale } from '@/i18n';
...
export function buildUidIndex(
  ...
  for (const post of posts) {
    const { uid, title } = post.data;
    const locale = getEntryLocale(post.id);
    if (!uid) continue;
...
export async function getPostTranslations(
  ...
  const existing = new Set(
    posts.map((p) => {
      const locale = getEntryLocale(p.id);
      return `${locale}/${localeStrippedSlug(p.id, locale)}`;
    })
  );
