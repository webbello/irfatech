...
/** The editor's raw choice, straight from frontmatter — "auto" (or unset) means no explicit choice was made. */
export type CoverType = 'auto' | 'svg' | 'image' | 'none';
/** What `resolveCoverVisual` actually decided to render. */
export type CoverVisual = 'svg' | 'image' | 'none';

/**
 * Which cover visual a post should render: the decorative SVG icon
 * (`svgSlug`), the uploaded cover `image`, or neither.
 *
 * `coverType` is an explicit editor choice (set from the CMS) and wins
 * whenever it names a visual that's actually available — so switching a
 * post to "Uploaded Image" shows the image immediately, without first
 * having to go clear out a leftover `svgSlug` value. Left unset (or set to
 * "auto", the CMS select's default option), it falls back to the original
 * implicit rule (SVG wins if both are present) so every post written
 * before this field existed keeps rendering exactly as it already does.
 */
export function resolveCoverVisual(
  coverType: CoverType | undefined,
  svgSlug: string | undefined,
  image: string | undefined
): CoverVisual {
  if (coverType === 'none') return 'none';
  if (coverType === 'image') return image ? 'image' : svgSlug ? 'svg' : 'none';
  if (coverType === 'svg') return svgSlug ? 'svg' : image ? 'image' : 'none';
  return svgSlug ? 'svg' : image ? 'image' : 'none';
}
