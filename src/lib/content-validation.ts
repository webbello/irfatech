 ...
 */
import { getEntryLocale } from '@/i18n';
...
/**
 * Build the list of published URLs from the blog, pages, projects, solutions,
 * and services collections. Blog posts live under `/blog/<slug>`; projects
 * under `/projects/<slug>`; solutions under `/solutions/<slug>`; services
 * under `/services/<slug>`; pages live at the site root `/<slug>`.
 *
 * Each entry's locale is derived from its id's folder segment (the single
 * source of truth for the locale collections) rather than read from
 * frontmatter, so an entry can never quietly claim a locale its folder
 * doesn't match; an id with no configured-locale prefix counts as the default
 * locale.
 */
export function collectSlugRecords(
  ...
  solutions: ContentEntryLike[] = [],
  services: ContentEntryLike[] = []
): SlugRecord[] {
  const records: SlugRecord[] = [];

  const record = (source: string, id: string, namespace: string): SlugRecord => {
    const locale = getEntryLocale(id);
    return { source, locale, path: `/${namespace}/${localeStrippedSlug(id, locale)}` };
  };

  for (const post of posts) {
    records.push(record(`blog: ${post.id}`, post.id, 'blog'));
  }

  for (const project of projects) {
    records.push(record(`projects: ${project.id}`, project.id, 'projects'));
  }

  for (const solution of solutions) {
    records.push(record(`solutions: ${solution.id}`, solution.id, 'solutions'));
  }

  for (const service of services) {
    records.push(record(`services: ${service.id}`, service.id, 'services'));
  }
 ...
 * within a locale. Drafts are excluded — they are not emitted in production,
 * so they cannot cause a real problem yet. Call this from a route's
 * `getStaticPaths`; a throw here aborts `astro build`.
 */
export async function assertNoSlugCollisions(): Promise<void> {
  const { getCollection } = await import('astro:content');
  const [posts, pages, projects, solutions, services] = await Promise.all([
    getCollection('blog'),
    getCollection('pages'),
  ...
  const publishableSolutions = solutions.filter((solution) => solution.data.draft !== true);
  const publishableServices = services.filter((service) => service.data.draft !== true);
  const collisions = findSlugCollisions(
    collectSlugRecords(
      publishablePosts,
      pages,
      publishableProjects,
      publishableSolutions,
      publishableServices
    )
  );
