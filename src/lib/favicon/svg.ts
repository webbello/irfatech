 ...
 * "Lexend" web font. Search-engine crawlers (and other renderers) do not load
 ...
 * against an embedded Lexend subset instead, so the SVG is self-contained.
 *
...
import { LEXEND_700_SUBSET_BASE64 } from './font-data.ts';
...
function getFont(): Font {
  if (!cachedFont) {
    const buf = Buffer.from(LEXEND_700_SUBSET_BASE64, 'base64');
  ...
  if (codePoint === undefined || !font.hasGlyphForCodePoint(codePoint)) {
    console.warn(
      `[favicon] No embedded glyph for "${letter}". Emitting a plain square. ` +
        `The embedded Lexend subset covers A-Z and 0-9; regenerate it if your ` +
