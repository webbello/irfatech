...
describe('Contact form validation', () => {
  ...
  it('accepts data with an optional service selected', () => {
    const result = contactSchema.safeParse({ ...validData, service: 'automation' });
    expect(result.success).toBe(true);
  });

  it('accepts data without optional service', () => {
    const result = contactSchema.safeParse(validData);
    expect(result.success).toBe(true);
  });
