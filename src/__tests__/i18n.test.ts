...
describe('i18n t() helper', () => {
  ...
  it('returns the Indonesian translation when locale is id', () => {
    expect(t('common.readMore', 'id')).toBe('Baca selengkapnya');
  });
  ...
  it('interpolates {placeholder} variables', () => {
    ...
    expect(t('blog.readingTime', 'id', { minutes: 5 })).toBe('5 menit baca');
  });
...
describe('i18n tData() helper', () => {
  ...
  it('returns the Indonesian structured value when locale is id', () => {
    const hero = tData<{ badge: string }>('pages.about.hero', 'id');
    expect(hero?.badge).toBe('Tentang');
  });
...
describe('i18n getSecondaryLocales()', () => {
  it('returns the non-default locales this site ships', () => {
    // Default config: enabled is true with locales ['en', 'id', 'pt', 'tet'],
    // so every locale but the default needs its own prefixed routes.
    expect(getSecondaryLocales()).toEqual(['id', 'pt', 'tet']);
  });
});

describe('i18n localizedPath()', () => {
  it('leaves the default locale unprefixed', () => {
    // The default locale always stays at the site root, i18n on or off.
  ...
  it('prefixes a secondary locale', () => {
    expect(localizedPath('/about', 'id')).toBe('/id/about');
  });
});
...
describe('i18n getLocaleFromPath()', () => {
  ...
  it('returns the default locale when the first segment is not a configured locale', () => {
    // 'nl' and 'zh-cn' are not among this site's configured locales
...
describe('i18n swapLocaleInPath()', () => {
  ...
  it('prefixes the path when swapping to a secondary locale', () => {
    expect(swapLocaleInPath('/about', 'id')).toBe('/id/about');
  });
});
...
describe('i18n meta titles never embed the site name', () => {
  ...
  // and this guards this site's own default dictionaries against a regression.
  const SITE_NAME = 'IRFAtech';
  ...
  const cases = ['en', 'id', 'pt', 'tet'].flatMap((locale) =>
