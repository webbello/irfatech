...
/**
 * Derive an entry's locale from its collection id. The i18n collections
 * (blog, projects, solutions, services) are stored one folder per locale
 * (`pt/`, `tet/`, ...), so the folder is the single source of truth — the
 * old `locale` frontmatter field duplicated it and could drift, silently
 * moving an entry between locales until a build-time check caught it. An id
 * not starting with a configured locale (a flat file, or a throwaway folder)
 * is treated as belonging to the default locale.
 */
export function getEntryLocale(id: string): Locale {
  const folder = id.split('/')[0];
  return isValidLocale(folder) ? folder : defaultLocale;
}
