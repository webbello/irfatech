...
// Maps the <select> value to the label the notification email should show —
// keeps the email readable even though the form only posts the value.
...
export const POST: APIRoute = async ({ request }) => {
  try {
    ...
    const subject = `[${siteLabel}] New contact from ${result.data.name}`;
    ...
    const { error } = await resend.emails.send({
      ...
      html: `
        ...
        ${result.data.service ? `<p><strong>Service:</strong> ${escapeHtml(SERVICE_LABELS[result.data.service] ?? result.data.service)}</p>` : ''}
